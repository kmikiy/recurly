...
func TestInvoices_List(t *testing.T) {
	...
	if err != nil {
	...
	} else if diff := cmp.Diff(invoices, []Invoice{{
		...
		State:            InvoiceStateOpenDeprecated,
		InvoiceNumber:    1005,
		DiscountInCents:  17,
		...
		CreditPayments: []CreditPayment{{
			...
			CreatedAt:             NewTimeFromString("2018-05-29T16:13:39Z"),
			UpdatedAt:             NewTimeFromString("2018-05-29T16:13:39Z"),
		}},
...
func TestInvoices_Get(t *testing.T) {
	...
	ts, _ := time.Parse(DateTimeFormat, "2018-06-05T15:44:57Z")
	if diff := cmp.Diff(invoice, &Invoice{
		XMLName:     xml.Name{Local: "invoice"},
		AccountCode: "1",
		Address: Address{
			Address: "400 Alabama St.",
		...
		UpdatedAt:        NewTime(ts),
		ClosedAt:         NewTimeFromString("2018-06-05T15:44:57Z"),
		DueOn:            NewTimeFromString("2018-06-05T15:44:57Z"),
		Type:             "charge",
		Origin:           "purchase",
		TaxRate:          float64(0),
		NetTerms:         NewInt(0),
		...
		LineItems: []Adjustment{
			{
				...
				Currency:          "USD",
				Taxable:           NewBool(false),
				StartDate:         NewTimeFromString("2018-06-05T15:44:56Z"),
				EndDate:           NewTimeFromString("2018-07-05T15:44:56Z"),
				CreatedAt:         NewTimeFromString("2018-06-05T15:44:57Z"),
				UpdatedAt:         NewTimeFromString("2018-06-05T15:44:57Z"),
			},
			{
				AccountCode:       "1",
				InvoiceNumber:     5558,
		...
		Transactions: []Transaction{
			{
				...
				CreatedAt: NewTimeFromString("2018-06-05T15:44:56Z"),
				Account: Account{
...
func TestInvoices_Create(t *testing.T) {
	...
	resp, invoice, err := client.Invoices.Create("10", Invoice{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	} else if resp.IsError() {
		t.Fatal("expected create invoice to return OK")
	} else if diff := cmp.Diff(invoice, &Invoice{
		XMLName:     xml.Name{Local: "invoice"},
		AccountCode: "1",
		Address: Address{
			Address: "123 Main St.",
		...
		TotalInCents:            3425,
		Currency:                "USD",
		DueOn:                   NewTimeFromString("2018-03-20T15:43:41Z"),
		CreatedAt:               NewTimeFromString("2018-03-19T15:43:41Z"),
		UpdatedAt:               NewTimeFromString("2018-03-19T15:43:41Z"),
		AttemptNextCollectionAt: NewTimeFromString("2018-03-20T15:43:41Z"),
		TaxType:                 "usst",
		TaxRegion:               "CA",
		NetTerms:                NewInt(0),
...
func TestInvoices_RefundVoidOpenAmount_Params(t *testing.T) {
	...
	// Fields ordered in same order as struct xml tags, XML above in same order
	// for equality check.
	resp, _, err := client.Invoices.RefundVoidOpenAmount(1010, 100, VoidRefundMethodCreditFirst)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	} else if resp.IsError() {
		t.Fatal("expected create open amount refund to return OK")
