...
func TestCreditPayments_List(t *testing.T) {
	...
	resp, creditPayments, err := client.CreditPayments.List(Params{"per_page": 1})
	if err != nil {
	...
	} else if diff := cmp.Diff(creditPayments, []CreditPayment{
		{
			XMLName:               xml.Name{Local: "credit_payment"},
			UUID:                  "3d3f6754c6df41b9d2a32e43029adc55",
			AccountCode:           "3465345645345",
			Action:                CreditPaymentActionPayment,
			Currency:              "USD",
			AmountInCents:         1000,
			OriginalInvoiceNumber: 1000,
			AppliedToInvoice:      1001,
			CreatedAt:             NewTimeFromString("2017-07-06T15:51:38Z"),
			UpdatedAt:             NewTimeFromString("2017-07-06T15:51:38Z"),
			VoidedAt:              NewTimeFromString("2017-07-22T15:51:38Z"),
		},
		{
			XMLName:                   xml.Name{Local: "credit_payment"},
			UUID:                      "3d3f6754c6df41b9d2a32e43029adc55",
			AccountCode:               "3465345645345",
			Action:                    CreditPaymentActionRefund,
			Currency:                  "USD",
			AmountInCents:             1000,
...
func TestCreditPayments_ListAccount(t *testing.T) {
	...
	resp, creditPayments, err := client.CreditPayments.ListAccount("1", Params{"per_page": 1})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	} else if resp.IsError() {
		t.Fatal("expected list credit payments to return OK")
...
func TestCreditPayments_Get(t *testing.T) {
	...
	if diff := cmp.Diff(creditPayment, &CreditPayment{
		XMLName:                   xml.Name{Local: "credit_payment"},
		UUID:                      "3d3f6754c6df41b9d2a32e43029adc55",
		AccountCode:               "3465345645345",
		Action:                    CreditPaymentActionRefund,
