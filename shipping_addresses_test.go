...
func TestShippingAddress_ListAccount(t *testing.T) {
	...
	r, shippingAddresses, err := client.ShippingAddresses.ListAccount("1", Params{"per_page": 1})
	if err != nil {
	...
	created, _ := time.Parse(DateTimeFormat, "2018-03-19T15:48:00Z")
	updated, _ := time.Parse(DateTimeFormat, "2018-03-19T15:48:00Z")

	if diff := cmp.Diff(shippingAddresses, []ShippingAddress{
		{
...
func TestShippingAddress_Create(t *testing.T) {
	...
	r, shippingAddress, err := client.ShippingAddresses.Create("1", ShippingAddress{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	} else if r.IsError() {
	...
	if diff := cmp.Diff(shippingAddress, &ShippingAddress{
		ID:          2438622711411416831,
		AccountCode: "1",
		Nickname:    "Work",
		FirstName:   "Verena",
...
func TestShippingAddress_Update(t *testing.T) {
	...
	r, shippingAddress, err := client.ShippingAddresses.Update("1", 2438622711411416831, ShippingAddress{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	} else if r.IsError() {
